        ...
        <div class="card-body">
            ...
            <form action="{{ route('supplier.update', $supplier->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group mb-3">
                    ...
                    <select name="kategori_id" class="form-control rounded-3 @error('kategori_id') is-invalid @enderror">
                        @foreach ($kategori as $k)
                            <option value="{{ $k->id }}" {{ old('kategori_id', $supplier->kategori_id) == $k->id ? 'selected' : '' }}>{{ $k->nama_kategori }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group mb-3">
                    ...
                    <input type="text" name="nama_supplier" class="form-control rounded-3 @error('nama_supplier') is-invalid @enderror" value="{{ old('nama_supplier', $supplier->nama_supplier) }}">
                    @error('nama_supplier') <span class="text-danger">{{ $message }}</span> @enderror
                </div>

                <div class="form-group mb-3">
                    <label class="fw-semibold">Barang dari Supplier</label>
                    <input type="text" name="barang_supplyan" class="form-control rounded-3 @error('barang_supplyan') is-invalid @enderror" value="{{ old('barang_supplyan', $supplier->barang_supplyan) }}">
                    @error('barang_supplyan') <span class="text-danger">{{ $message }}</span> @enderror
                </div>

                <div class="col-md-6">
                    <label class="fw-semibold">Kontak Supplier</label>
                    <input type="number" name="kontak_supplier" class="form-control rounded-3 @error('kontak_supplier') is-invalid @enderror" value="{{ old('kontak_supplier', $supplier->kontak_supplier) }}">
                    @error('kontak_supplier') <span class="text-danger">{{ $message }}</span> @enderror
                </div>
